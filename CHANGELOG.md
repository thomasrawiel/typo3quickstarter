...
- `install.sh` installs the script as a system-wide command - `typo3quickstarter`, with `t3quickstarter` as a short alias - so instances can be created from any directory without keeping a copy of the script next to them. Installs to `~/.local/bin` by default (`--prefix=DIR` for anywhere else), with `uninstall.sh` as its counterpart. Both refuse to overwrite or delete a same-named command they didn't install, and work both from a checkout and piped straight from `curl`. The usage text and the "To clean up this instance" hint now print whichever name the script was called under - see [docs/installation.md](docs/installation.md) ([#9](https://github.com/pagea-dev/typo3quickstarter/issues/9)).
